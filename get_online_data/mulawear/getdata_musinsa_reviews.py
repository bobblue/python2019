...
def get_reviews(soup_data, goods_no):
    ...
    soup_star = soup_data.select('div.profile > p')
    for i in soup_star:
        starinfo = i.select('span')[2]
        starinfo = str(starinfo)
        p = re.compile('score_\d+')
        star = str(p.findall(str(starinfo)))
        star = star.replace('[\'score_', '')
        star = star.replace('\']', '')
        star_list.append(float(star)*0.1)


    df = pd.DataFrame(data= {'product_no': goods_no, 'date':date_list, 'title':title_list, 'content':content_list, 'option':option_list, 'star':star_list})
    #print(df)
    return df 
    
def main():
    dataset = pd.read_excel("C:/Users/leevi/Desktop/데상트_2월/무신사크롤링/백데이터/무신사_크롤링_운동화.xlsx", encoding='utf-8')
    ...
    df_all.to_excel('C:/Users/leevi/Desktop/데상트_2월/무신사_크롤링_댓글_운동화.xlsx', index=False)
    print('저장 완료! ')
