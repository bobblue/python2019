...
filepath = 'C:/Users/leevi/Desktop/데상트_2월/나이브베이즈/'
dictname = 'mypolarity_for_reviews_newone'
filename = 'testdata_디스커버리'

df_dic = pd.read_excel("{}/{}.xlsx".format(filepath, dictname))


df = df_dic[df_dic['value'].notnull()]
df = df[['feed_word','value']]
...
for i, row in df.iterrows():
    if row['value'] ==  'p':
        pos_dic.extend(re.findall(p, row['feed_word']))
    elif row['value'] ==  'n':
        neg_dic.extend(re.findall(p, row['feed_word']))
    elif row['value'] ==  'neu':
        neu_dic.extend(re.findall(p, row['feed_word']))
...
# 결과 출력시키기
print('긍정의 문장 수는 ' + str(len(pos_word)) + '개 입니다')
print('부정의 문장 수는 ' + str(len(neg_word)) + '개 입니다')
print('중립의 문장 수는 ' + str(len(neu_word)) + '개 입니다')

print('긍정인 반응은 ' + str(len(pos_word) / (len(pos_word) + len(neg_word) + len(neu_word))) + '% 입니다')
print('부정인 반응은 ' + str(len(neg_word) / (len(pos_word) + len(neg_word) + len(neu_word))) + '% 입니다')
print('중립인 반응은 ' + str(len(neu_word) / (len(pos_word) + len(neg_word) + len(neu_word))) + '% 입니다')
